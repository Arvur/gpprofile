...
type
  ...
  //public
    procedure Access(instance: TPersistent);
...
function SetToString(const aSet; TypeInfo: PTypeInfo; brackets: boolean = true): string;
...
begin
{$IFNDEF HasWideStrProp}
  GetWideStrProp(Instance, GetPropInfo(Instance, PropName), Result);
{$ELSE}
  Result := GetWideStrProp(Instance, GetPropInfo(Instance, PropName));
{$ENDIF HasWideStrProp}
end; { GetWideStrProp }
...
begin
  SetWideStrProp(Instance, GetPropInfo(Instance, PropName), Value);
end; { SetWideStrProp }
...
var
  ...
  S          : TIntegerSet;
begin
  integer(S) := integer(aSet);
  eltTypeInfo := GetTypeData(TypeInfo).CompType^;
  for i := GetTypeData(eltTypeInfo).MinValue to GetTypeData(eltTypeInfo).MaxValue do
...
begin
  Cleanup;
  gpInstance := instance;
  gpNumProperties := TypInfo.GetTypeData(instance.ClassInfo)^.PropCount;
  if gpNumProperties > 0 then begin
    GetMem(gpPropList, Count*SizeOf(pointer));
    GetPropInfos(instance.ClassInfo, gpPropList);
  end;
...
begin
  Result := PropInfo[idxProperty]^.Name;
