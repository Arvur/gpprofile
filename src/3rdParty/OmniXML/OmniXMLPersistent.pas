{ $OmniXML: OmniXML/OmniXMLPersistent.pas,v 1.9 2007/02/11 20:58:36 mremec Exp $ }
...
type
  ...
  protected
    ...
    procedure InternalWriteText(Root: IXMLElement; Name, Value: WideString);
  ...
  public
    ...
    class procedure SaveXML(const Instance: TPersistent; var XML: WideString;
      const PropFormat: TPropsFormat = pfAuto; const OutputFormat: TOutputFormat = ofNone);
  ...
  protected
    ...
    function InternalReadText(Root: IXMLElement; Name: String;
...
class procedure TOmniXMLWriter.SaveXML(const Instance: TPersistent; var XML: WideString;
...
procedure TOmniXMLWriter.InternalWriteText(Root: IXMLElement; Name, Value: WideString);
var
  ...
  begin
    ...
    if WriteDefaultValues or (Value <> PPropInfo(PropInfo)^.Default) then begin
      case PropType^.Kind of
        ...
        tkWChar: InternalWriteText(Element, PPropInfo(PropInfo)^.Name, WideChar(Value));
        tkSet: InternalWriteText(Element, PPropInfo(PropInfo)^.Name,
...
function TOmniXMLReader.InternalReadText(Root: IXMLElement; Name: String;
  var Value: WideString): Boolean;
var
  PropNode: IXMLElement;
  AttrNode: IXMLNode;
...
var
  ...
  var
    Value: WideString;
  begin
    if not InternalReadText(Element, PPropInfo(PropInfo)^.Name, Value) then
      Value := '';
