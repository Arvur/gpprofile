...
$Id: SynHighlighterPHP.pas,v 1.1.1.1 2004/02/20 08:18:25 anoncvs Exp $
...
begin
  ...
  repeat
    case FLine[Run] of
      #0, #10, #13: break;
      '\': if FLine[Run + 1] in [#39, '\'] then inc(Run);
    end;
    inc(Run);
  until FLine[Run] = #39;
  ...
  repeat
    case FLine[Run] of
      ...
      #92:                             {backslash}
        ...
        if (FLine[Run + 1] in ['\',#34]) then inc(Run);                         //DDH Addition from Eden Kirin
    end;
    inc(Run);
  until FLine[Run] = #34;
